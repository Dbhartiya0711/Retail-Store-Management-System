...
<!-- Compiled and minified CSS -->
	...
	<%
    		...
    		String billno="";
    		String Query111 ="select count(*) from purchase";
    		ResultSet rs111= st.executeQuery(Query111);
            rs111.next();
            if(rs111.getInt(1)<1)
            {
            	out.println("<script type=\"text/javascript\">");
			   	out.println("alert('Cannot add Return until Sale Bill is Added.');");
			   	out.println("location='salereturns.jsp';");
			   	out.println("</script>");
            }
            else
            {
			...
			else
			{
				...
				%>
						...
						if(co.localeCompare(s)==0)
						{
							...
							document.getElementById("discount "+n[1]).value=<%=dis%>.toFixed(2);
							settotalq();
				...
				setamount(x);
				<%
                String Query17 ="select * from sale_details where bill_no="+billno;
	    		ResultSet rs17= st.executeQuery(Query17);
	    		double tot=0;
                while(rs17.next())
	    			tot = tot +rs17.getDouble(5);
                String Query18 ="select total from sale where bill_no="+billno;
	    		ResultSet rs18= st.executeQuery(Query18);
	    		rs18.next();
                double ne= rs18.getDouble(1);
                double disc=((tot-ne)/tot)*100;
                %>
		...
		function setcustomercon()
		{
			...
			else
			{
				<%
		    		...
		    		while(rs13.next())
		    		{
		    			String c=rs13.getString("contact");
  ...
  <div class="nav-wrapper head1 indigo">
    ...
    <ul>
     ...
     <a href="../logoutdata.jsp">Sign Out</a>
			...
			<h3 class="center">Add Sale Return</h3>
			<br><br>
			<div >
			<form action="asalereturndata.jsp" method="post">
				
				
				<div style="margin:0px 50px 0px 421px;display:inline-block;" >
					<label for="billno" style="font-size:15px;display: inline-block;">Return for bill no.</label>
					<select class="browser-default" onchange="setcustomeridbybill(),showhide(),addField();" id="billno" name="billno" style="display: inline-block;font-size:15px;width: 150px;height: 30px;" required="required" >
					...
					<label for="datepicker" style="font-size: 15px;margin-left:20px;">Date Of Return Bill</label>
					...
					<br>
					<label style="font-size: 15px;">Customer Id</label>&ensp;
					<input onchange="setcustomername()" id="customerid" name="customerid" style="display: inline-block;font-size:15px;width: 150px;height: 30px;" readonly="readonly"/>			
					<label style="font-size: 15px;">Customer Mobile</label>&ensp;
					<input id="customercon" name="customercon" style="font-family: 'raleway';font-size:18px;width: 150px;height: 25px;padding-left: 10px;background: white;" readonly="readonly"/>
					
					<label style="font-size: 15px;">Customer Name</label>&ensp;
				...
				<br/>
				<div>
				<table id="myTable" class="center" style="display: none">
				...
				<div>
				<table id="myTable1" class="center" style="display: none;">
					<tr>
		...
		<br>
		<div id="goaway" style="display:block;">

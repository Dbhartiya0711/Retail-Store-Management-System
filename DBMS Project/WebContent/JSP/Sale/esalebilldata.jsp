...
<body>
	...
	<%
		...
		String Query1 ="select count(*) from sale where Bill_no="+billno;
		...
		if(a==1)
		{
			...
			String Query2 ="select * from sale_details where Bill_no="+billno;
			...
			if(h!=0)
			{
				...
				String Query4 ="delete from sale_details where Bill_no="+billno;
				int f= st2.executeUpdate(Query4);
				if(f!=0)
				{
					String Query5 ="delete from sale where Bill_no="+billno;
					int g= st.executeUpdate(Query5);
					if(g==1)
					{
						String Customerid = request.getParameter("Customerid");
						String total = request.getParameter("net");
						String billdate = request.getParameter("billdate");
						Class.forName("com.mysql.jdbc.Driver");
						Connection con1 = DriverManager.getConnection("jdbc:mysql://localhost:3306/rsms","root","root");
						Statement st3 = con1.createStatement();
						String Query6 ="select count(*) from sale where Bill_no="+billno;
						ResultSet rs3= st.executeQuery(Query6);
						rs3.next();
						int ab=rs3.getInt(1);
						if(ab==0)
						{
							String Query7 ="insert into sale values('"+billno+"','"+Customerid+"','"+total+"','"+billdate+"')";
							...
							if(fg==1)
							{
								...
								{
									String Query8 ="insert into sale_details values('"+billno+"','"+code[i]+"','"+quantity[i]+"','"+price[i]+"','"+amount[i]+"','"+expiry[i]+"')";
								...
								out.println("<script type=\"text/javascript\">");
							   	out.println("alert('Sale Bill Updated!!!');");
